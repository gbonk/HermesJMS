	...
	private synchronized void checkConnected() throws JMSException {
		if (!connected) {
			try {
				...
				AdminModule.connect(factory.getHostname(), factory.getPort(), factory.getUsername(),
						factory.getPassword(), factory.getCnxTimer());
			...
			try {
				...
				for (int i = 0; i < ids.length; i++) {
					...
					for (int j = 0; j < subs.length; j++) {
						rval.put("subscriber " + j,
								ids[i] + " : " + subs[j].getName() + "; Durable:" + subs[j].isDurable());
